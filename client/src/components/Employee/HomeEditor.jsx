...
const HomeEditor = () => {
  ...
  const submitStatus = async (id) => {
    try {
      const subSta = await requestApi.updateStatus(
        auth.accessToken,
        id,
        status
      );
  ...
  useEffect(() => {
    if (requestId !== "" && imageUrls.length === imageUploads.length) {
      ...
      const performAddRequest = async () => {
        try {
          const requestData = await requestApi.updateNewPhoto(
            auth.accessToken,
            requestId,
            imageUrls[0],
            status
          );
          console.log(requestData.statusCode);
    ...
    <>
      ...
      <div className="w-full">
        <div className="flex items-center justify-center border border-black">
          <img
            src="https://firebasestorage.googleapis.com/v0/b/fotofushion-51865.appspot.com/o/Products%2FbannerEditor.png?alt=media&token=1666224e-7e7c-454c-a307-64eb9bbe85b7"
            alt=""
          />
        </div>
        <div className="flex items-center justify-center">
          <h1 className="font-bold text-5xl m-10">
            Bảng Công Việc Của Chỉnh Sửa Hình Ảnh
          </h1>
        </div>
        <div className="flex items-center justify-center">
          <div className="w-3/4">
        ...
        <dialog id="my_modal_2" className="modal">
          <div className="modal-box">
            <img
              ...
              src={
                selectedItem
                  ? selectedItem.img_url_old
                  : "https://daisyui.com/images/stock/photo-1567653418876-5bb0e566e1c2.jpg"
              }
            />
